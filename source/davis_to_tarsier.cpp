...
#include "davis_reader.qml"
  ...
  // run event polling from camera in separate thread to allow GUI to render
  auto _loop = std::thread([&]() {
    while (!globalShutdown.load(memory_order_relaxed)) {
      ...
      for (auto &packet : *packetContainer) {
        ...
        // we only process change detection events
        if (packet->getEventType() == POLARITY_EVENT) {
          std::shared_ptr<const libcaer::events::PolarityEventPacket> polarity =
              std::static_pointer_cast<libcaer::events::PolarityEventPacket>(
                  packet);

          // create handler callbacks, afterwards callable via overloaded ()
          auto select_rectangle = tarsier::make_my_filter<sepia::dvs_event>(
              20, 20, 200, 160,
              [&](sepia::dvs_event event) { dvs_display->push(event); });

          // process each event individually and pass it to handler
